...
class SQLAlchemyPostRepo(IPostRepository):
    posts = list()
    no_posts = 0
    def __init__(self, db:IDatabaseAlchemy):
        self.db = db
    def get_all(self,per_page, offset, selected_owner_id):
        ...
        join(UserSQLAlchemy).filter(case([(selected_owner_id != -1,PostSQLAlchemy.owner_id == selected_owner_id )],
        else_=PostSQLAlchemy.owner_id > 0)).order_by(desc(PostSQLAlchemy.post_date_creation)).offset(offset).limit(per_page).all()
        self.posts.clear()
    ...
    def create(self, post:Post):
        ...
        stmt = (insert(posts_table).values(post_title = post.post_title, post_content =post.post_contents,owner_id = post.owner_id ).returning(posts_table.c.post_id))
    ...
    def update(self, post:Post):
        conn = self.db.create_conn()
        posts_table = PostSQLAlchemy.__table__
        stmt = (update(posts_table).where(posts_table.c.post_id == post.post_id).values(post_title = post.post_title, post_content = post.post_contents))
        conn.execute(stmt)
        self.db.close_and_save(conn)
