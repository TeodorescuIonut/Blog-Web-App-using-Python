...
class PostDbRepo(IPostRepository):
    ...
    def get_all(self, per_page, offset, selected_owner_id) -> []:
        ...
        cur.execute(
            f"SELECT post_id,post_created_on,post_modified_on, post_title, LEFT(post_content, 500), owner_id, "
            f"user_name,COUNT(*) OVER() AS full_count FROM posts INNER JOIN users ON owner_id = user_id {query} ORDER "
            f"BY post_created_on DESC OFFSET {offset}  LIMIT {per_page}")
        rows = cur.fetchall()
        self.posts.clear()
    ...
    def get_by_id(self, post_id: int) -> Post:
        ...
        cur = conn.cursor()
        cur.execute(
            "SELECT post_id, to_char(post_created_on, 'dd/mm/yyyy HH24:MI'),to_char(post_modified_on, 'dd/mm/yyyy "
            "HH24:MI'), post_title, post_content, owner_id, user_name  FROM posts INNER JOIN users ON owner_id = "
            "user_id  WHERE post_id = %s",
            (post_id,))
        data = cur.fetchone()
        post = Post("", "", "", "")
        if data is not None:
